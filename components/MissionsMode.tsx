...
function ShareCard({ mission, turnsUsed }: { mission: Mission; turnsUsed: number }) {
  ...
  const linkedInTitle = `I just attacked ${agentName} on Agent Disaster Lab`
  const linkedInSummary = `Mission: ${mission.title} (${mission.difficulty}) · ${turnsUsed} attack${turnsUsed !== 1 ? "s" : ""} · +${mission.points} pts · comply54 blocked every attempt.`
  const linkedInUrl = `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent("https://disaster.comply54.io")}&title=${encodeURIComponent(linkedInTitle)}&summary=${encodeURIComponent(linkedInSummary)}`
    ...
    <div className="rounded-xl border border-white/8 bg-white/[0.02] px-5 py-4 space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#0A66C2]/30 bg-[#0A66C2]/[0.06] text-xs text-[#0A66C2]/70 hover:text-[#0A66C2] hover:border-[#0A66C2]/50 hover:bg-[#0A66C2]/[0.12] transition-all"
        >
